...
import { getCartProducts, updateCartSuccess, updateCartdata } from '../redux/Cartreducer/action'
...
{data?.map((item)=>(
    ...
    <Grid ml="" templateColumns="repeat(3,1fr)" >
        ...
        <Box gridColumnStart={2} gridColumnEnd={3} mt="2%" >
           ...
           <Flex  mt="5%" gap="5%" textAlign={"left"} fontWeight={"600"} fontSize={"1.2vw"}>
           <Text>Price:</Text>
            <Text textAlign={"left"}>Rs.{item.price}</Text></Flex> 
            <Flex mt="5%" gap="5%" textAlign={"left"} fontWeight={"600"} fontSize={"1.1vw"}>
            <Text>Rating:</Text>
...
<Flex mt="5%"  ml="5%" fontWeight={600} gap="10%">
    <Text  fontSize={"1.1vw"}>Category:</Text>
    <Text fontSize={"1.1vw"}>{item.categories}</Text>
</Flex>
<Flex mt="5%" ml="5%" fontWeight={600} gap="10%">
    <Text  fontSize={"1.1vw"}>Color:</Text>
    <Text fontSize={"1.1vw"}>{item.color}</Text>
</Flex>
<Button  mt="20%" w="60%" bg="blackAlpha.900"  color={"white"} >Remove</Button>
